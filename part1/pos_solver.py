...
class Solver:
    ...
    def hmm_viterbi(self, sentence):
        ...
        if len(sentence) == 1:
            label_list = []
            # max of posterior ka index
            # us index ka POS value ko label list mi append krna h
            ...
            for col in range(1, len(sentence)):
                ...
                for row in range(len(self.transition_matrix)):   # Loop through 12 POS rows
                    state_posterior = []
                    #print(self.transition_matrix[row][col])
                    ...
                    #posterior_prob[row] = min(state_posterior, key = lambda x:x[1])[1]
                    prev_edge = min(state_posterior, key = lambda x:x[1])
                    #print("prev edge: ",prev_edge)
                    possible_col.append(prev_edge)
                #print("possible column: ",possible_col)
            ...
            #print("rows of path matrix: ", len(path_matrix))
            #print("col of path matrix: ", len(path_matrix[0]))
            #Backtrack the path matrix
